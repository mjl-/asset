/*
Package asset is a simple way to embed arbitrary files in your go binary.
All you have to do is append a zip file to your binary.  Typically on
launch of your program, you call asset.Fs() to get a handle to the
vfs.FileSystem that represents the appended zip file.  It is on this vfs
that you should call Open and friends, as opposed to the normal os.Open.
...
An example:
	...
	func main() {
		...
		// all vfs operations return an error.
		fs := asset.Fs()
		if err := asset.Error(); err != nil {
			log.Fatal(err)
		}

		// note that paths passed to fs.Open must be absolute, i.e. starting with a slash.
		...
		// vfs (inside the godoc package) has a convenient function OS(root).
		// It returns a vfs.FileSystem that reads from the local filesystem.
		// try:  fs = vfs.OS(".")
	}

Build your program, let's say the result is "mybinary".
Now create a zip file, eg on unix: zip -r0 mybinary.zip test.txt.
Append it to the binary:  cat mybinary.zip >>mybinary.
If you run mybinrary, it will print the contents of file "test.txt"
in the zip file that you appended to the binary.

To make this work, an assumption about zip files is made:
That the central directory (with a list of files inside the zip file) is
contiguous and comes right before the "end of central directory" marker.
This is almost always the case with zip files.  With this assumption,
asset can locate the start and end of the zip file that is appended to
the binary, which archive/zip needs in order to parse the zip file.
...
This has been tested with binaries on Linux (Ubuntu 12.04 LTS), Mac OS X
(10.9.2) and Windows 8.1.  It appears that these operating systems don't
mind extra data at the end of the binary.
*/
package asset
...
// we assume the central directory is contiguous, just before the end-of-central-directory.
